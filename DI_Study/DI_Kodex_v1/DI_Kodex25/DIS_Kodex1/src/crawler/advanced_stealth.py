...
def _desktop_user_agent() -> str:
    """
    Attempts to return a desktop-class user agent string to avoid
    being redirected to mobile pages.
    """
    for _ in range(5):
        try:
            candidate = ua.chrome
            ...
            break
        if candidate and "Mobile" not in candidate and "Android" not in candidate:
            return candidate
    # Fallback to a sturdy desktop UA
    return (
